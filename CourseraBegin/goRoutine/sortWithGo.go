...
func main() {
	...
	sorted_slice := make([]int, len(slice))
	sz := len(slice)
	slice1 := slice[0 : sz/4]
	slice2 := slice[sz/4 : sz/2]
	slice3 := slice[sz/2 : sz*3/4]
	slice4 := slice[sz*3/4 : sz]
	...
	wg.Wait()
	go mergeSlices(slice1, slice2, slice3, slice4, sorted_slice)
	fmt.Println(slice1, slice2, slice3, slice4, sorted_slice)
